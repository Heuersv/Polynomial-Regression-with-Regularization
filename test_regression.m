...
degree = 6;
regularization_factor = 100;
edges = [0, 365];
%edges = [min(timestamps), max(timestamps)];

% Do the regression
coefficients = polynomial_regression(timestamps, ndvi_values, degree, regularization_factor, edges);

% Plot result
scatter(timestamps, ndvi_values)
hold on
fplot(@(x) polyval(fliplr(coefficients'), x), edges)
hold off